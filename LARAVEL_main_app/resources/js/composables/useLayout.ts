...
import { initializedThemeStore } from "./usePinia";
...
function setAppStateDark(theme: string | null = defaultTheme) {
    if(theme === "dark") {
...
export function setAppState(){
    appState.value.primary = primaryColor;
    appState.value.surface = surfaceColor;
    setAppStateDark();
    
    const primary = primaryColors.value.find(c => c.name === primaryColor);
    const surface = surfaces.value.find(c => c.name === surfaceColor);
...
export function useLayout() {
    const themeStore = initializedThemeStore();
    ...
    function setSurface(value: string) {
        ...
        themeStore.setSurfaceColor(value);
    }

    function toggleDarkMode() {
        if(appState.value.darkMode) {
            themeStore.setThemeLS();
        }else{
            themeStore.setThemeLS("dark");
        }
