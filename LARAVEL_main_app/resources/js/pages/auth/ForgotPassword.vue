...
const submitValue = ref("Reset Password");
...
const reset = () => {
    form.post(route('forgot-password.request'), {
        onProgress: () => {
            ...
            submitValue.value = "Checking Email";
        },
        onError: () => {
            isEmailChecked.value = false;
            submitValue.value = 'Reset Password';
        },
        onSuccess: (response) => {
            isEmailChecked.value = true;
            submitValue.value = 'Email Ditemukan';
            form.email = String(response?.props?.email ?? '');
            console.log(response.props);
        }
    });
};
const updatePassword = () => {
    form.post(route('forgot-password.update_password'), {
        onSuccess: () => {
            isEmailChecked.value = false;
            submitValue.value = 'Reset Password';
        }
...
<template>
    ...
    <div class="min-h-screen flex flex-col items-center justify-center bg-primary-200 dark:bg-surface-950 text-surface-600 dark:text-surface-0 font-medium">
        <div class="w-md main-wrapper rounded-lg drop-shadow-xl shadow-2xl overflow-hidden">
            ...
            <div class="default-transition overflow-hidden" :class="{'max-h-0': isEmailChecked, 'w-full max-h-screen' : !isEmailChecked}">
                <div class="p-5">
                    <div class="flex flex-col mb-3">
                        <h1 class="w-fit text-2xl font-semibold mb-1">Forgot Password</h1>
                        <h1 class="font-light">Please enter your email address you would like to reset the password</h1>
                    </div>
                    <div class="card">
                        <form @submit.prevent="reset">
                            <div class="flex flex-col gap-2">
                                <div class="flex flex-col gap-1">
                                    <InputText v-model="form.email" type="email" class="w-full" id="email" placeholder="Email" required :disabled="isEmailChecked" />
                                    <InputError :message="form.errors.email" />
                                </div>
                                <Button type="submit" class="w-full bg-primary hover:bg-primary-0 my-2 font-semibold" :disabled="isEmailChecked || form.processing">{{ submitValue }}</Button>
                            </div>
                        </form>
                        <div class="w-fit m-auto text-sm text-center mt-4 text-primary dark:text-surface-0 hover:text-primary-300 transition-colors duration-300">
            ...
            <div class="default-transition overflow-hidden" :class="{'max-h-0': !isEmailChecked, ' w-full max-h-screen' : isEmailChecked}">
                <div class="flex items-center w-fit rounded gap-1 ml-5 mt-4 cursor-pointer text-primary dark:text-surface-0 hover:text-primary-300 transition-colors duration-300" @click="back">
                    <i class="pi pi-arrow-left"></i>
                    <p>Back</p>
                </div>
                <div class="p-5">
                    <div class="flex flex-col mb-3">
                        <h1 class="w-fit text-2xl font-semibold mb-1">Reset Password</h1>
                        <h1 class="font-light">Please enter your new password for your account</h1>
                        <h1 class="text-green-600 dark:text-green-400">{{ form.email }}</h1>
                    </div>
                    <div class="card">
                        <form @submit.prevent="updatePassword">
                            <div class="flex flex-col gap-3">
                                <div class="flex flex-col gap-1">
                                    <InputText v-model="form.password" type="password" class="w-full" id="password" placeholder="Password" required/>
                                    <InputError :message="form.errors.password" />
                                </div>
                                <div class="flex flex-col gap-1">
                                    <InputText v-model="form.password_confirmation" type="password" class="w-full" id="password_confirmation" placeholder="Password Confirmation" required/>
                                    <InputError :message="form.errors.password_confirmation" />
                                </div>
                                <Button type="submit" class="w-full bg-primary hover:bg-primary-0 my-2 font-semibold">Submit</Button>
                            </div>
                        </form>
                        <div class="w-fit m-auto text-sm text-center mt-4 text-primary dark:text-surface-0 hover:text-primary-300 transition-colors duration-300">
                            <div class="flex items-center gap-2">
